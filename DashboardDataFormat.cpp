...
void sendVisionData(double lefto,double rito,double lefbo, double ritbo) {
	...
	dash.AddCluster(); 			// wire (2 elements)
	{
		...
		dash.AddDouble(lefto);	// All four changed from Int to Double
		dash.AddDouble(rito);
		dash.AddDouble(lefbo);
		dash.AddDouble(ritbo);
